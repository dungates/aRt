...
pi <- tibble(
  pi = read_lines(here("Data", "pi.txt")) %>% str_remove("\\.")
) 
...
pi_graph %>% 
  ...
  annotate(
    ...
    family = "Baskervville", size = 64, color = "white"
  ) +
  ...
  theme_void(base_family = "Mesmerize Regular") +
  theme(
  ...
  ggsave(here("Images/PieNetwork.png"))

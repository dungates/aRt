...
im <- load.image(here("Images/tree.png"))
...
plot <- filter(plot_data, year %in% seq(2015, 2210, 10)) %>%
  ...
  geom_text(data = labels, aes(x = 11, y = 17, label = label), family = "Antonio", size = 8) +
  labs(x = NULL,
       y = NULL,
       ...
       caption = "**Data**: ourworldindata.org/forests | **Article**: doi.org/10.1038/nature14967 | **Graphic**: @gates_duncan") +
  scale_fill_identity() +
  coord_equal() +
  scale_y_reverse() +
  scale_x_continuous(limits = (c(-30,30))) +
  theme_void() +
  theme(plot.title = element_text(hjust = 0.6, family = "Antonio"),
...
anim_save(here('Images', 'trees_dying.gif'), animation = anim + exit_fade(), renderer = gifski_renderer(loop = FALSE), width = 8, height = 8, units = 'in', res = 150)

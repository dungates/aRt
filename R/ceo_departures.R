...
invol_legal <- departures %>% 
  ...
  mutate(
    w = round(runif(1, 0, 3)),
    h = runif(1, 0, 3),
    img = here("Images", paste0("ceo", round(runif(1, 1, 7)), ".png"))
  ) %>% 
  ungroup()
...
geom_polygon(data = windows, aes(x = x - 12.75, y = y - 2, group = interaction(c, r)), fill = "lightblue", color = "darkblue") +
  # ceo ----
geom_point(aes(x = -8.25 - w, y = fyear - w * 0.4, size = n*10), shape = 8, color = "darkblue") +
...
ggsave(here::here("Images", paste0("departures-", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png")), dpi = 320, width = 9, height = 9)

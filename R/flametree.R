...
# flametree_plot(dat)
(p <- flametree_plot(dat, background = "black", palette = "scico::berlin") +
    scale_y_reverse())
(p2 <- flametree_plot(dat, background = "black", palette = "scico::berlin"))

(p3 <- flametree_plot(dat, background = "transparent", palette = "scico::berlin") +
    scale_y_reverse())
(p4 <- flametree_plot(dat, background = "transparent", palette = "scico::berlin"))
...
ggdraw(patchwork) +
  draw_plot(p3, x = 0, y = 0, scale = 0.165) + 
  draw_plot(p4, x = 0, y = 0, scale = 0.165) +
  draw_plot(p3, x = 0, y = -0.15, scale = 0.45) + 
  draw_plot(p4, x = 0, y = 0.15, scale = 0.45)

# ggdraw(patchwork) + draw_plot(p3, x = 0, y = 0, scale = 0.25)

ggsave(here("Images/TripleFlametree.png"))
knitr::plot_crop(here("Images/TripleFlametree.png"))

possible_colors <- palette_c_names


source(here("R/flametree_source.R"))
flametree_grow(seed = 1, trees = 10, time = 4, split = 2, angle = c(10,-50,50)) %>% 
  flametree_plot(style = "voronoi", background = "beige") +
  theme(panel.background = element_rect(fill = "beige", color = NA))

ggsave(here("Images/EarthyTree.png"))
knitr::plot_crop(here("Images/EarthyTree.png"))
...
make_ggplot <- function(param, raster, dat) {
  ggplot2::ggplot(
    ...
    ggplot2::scale_color_manual(values = scico::scico(n = 31, palette = "berlin")) +
...
img <- turmite60(shade = "#9EB0FF", slice_n = 5, heart_size = 0.41, heart_shift_x = 0.49, heart_shift_y = 0.55, palette_n = 30)

# cowplot::ggdraw(p) +
#   draw_plot(img, 0.175, 0.1, 0.3, 0.3)

ggdraw(img) +
  draw_plot(p, 0.25, 0.25, x = 0.5, y = 0.5)
